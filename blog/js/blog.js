...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const animateCounter = (element, target, duration = 2000) => {
    const start = 0;
    const increment = target / (duration / 16); // 每 16ms 增加的數值
    let current = start;

    const timer = setInterval(() => {
      current += increment;
      if (current >= target) {
        element.textContent = target; // 確保最終顯示目標值
        clearInterval(timer);
      } else {
  ...
  const createScrollProgress = () => {
    ...
    progressBar.style.cssText = `
        ...
        left: 0;
        height: 3px;
        background: var(--gradient-progress, linear-gradient(90deg, #C4A35A, #7A6252));
        z-index: 9999;
