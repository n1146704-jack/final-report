...
function copyCode(button) {
  ...
  navigator.clipboard.writeText(text).then(() => {
    ...
    button.style.background = 'var(--color-success, #5A6B52)';
...
function generateTableOfContents() {
  ...
  if (headings.length === 0) {
    tocContainer.innerHTML = '<p style="color: var(--text-muted, #9C8B7A); font-size: 0.875rem;">無可用目錄</p>';
    return;
  }
...
function createShareModal(url) {
  ...
  modal.innerHTML = `
        ...
        <input type="text" value="${url}" readonly style="width: 100%; padding: 0.5rem; margin-bottom: 1rem; border: 2px solid var(--cream-dark, #D4C4A8); border-radius: 0.25rem;">
        <button onclick="this.closest('div').remove()" style="padding: 0.5rem 1rem; background: var(--earth-primary, #7A6252); color: white; border: none; border-radius: 0.25rem; cursor: pointer;">關閉</button>
    `;
...
function showNotification(message, type = 'success') {
  ...
  // 使用 CSS 變數，提供備援值
  const bgColor = type === 'success' ? 'var(--color-success, #5A6B52)' :
    type === 'warning' ? 'var(--color-warning, #B8860B)' :
      'var(--earth-primary, #7A6252)';
